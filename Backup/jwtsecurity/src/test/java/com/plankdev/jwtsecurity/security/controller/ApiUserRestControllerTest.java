...
public class ApiUserRestControllerTest { //TODO: extend from BaseRestControllerTest
    ...
    public void updateUserSuccessWithAdminRole() throws Exception {
        ...
        ApiUser apiUser = new JsonUtils(mappingJackson2HttpMessageConverter).jsonStringToPojo(userJsonAsString, ApiUser.class);
        apiUser.setFirstName("firstName");

        ResultActions resultActions = mockMvc.perform(put("/api/users/1")
                .contentType(contentType)
                .content(json(apiUser)));
    ...
    public void deleteUserSuccessWithAdminRole() throws Exception {
        ApiUser apiUser = createNewUser();

        ResultActions resultActions = mockMvc.perform(delete("/api/users/" + apiUser.getId()))
                .andDo(print());
    ...
    private ApiUser createNewUser() throws Exception {
        ApiUser apiUser = new ApiUser();
        apiUser.setUsername("testUser2");
        apiUser.setPassword("password");
        ...
        return new JsonUtils(mappingJackson2HttpMessageConverter).jsonStringToPojo(userJsonAsString, ApiUser.class);
    }
    ...
    private String passwordIgnoreWorkaround(String userJson) {
        StringBuffer buffer = new StringBuffer(userJson);
